...
function Erettsegi(pontok:number[]):[number,number]{
    ...
    else if(eredmeny<=119){
        return [eredmeny,4];
    }
    else if(eredmeny<=150){
        return [eredmeny,5];
    }
}
...
//6. feladat
let snookerInfo:string[]= ["52;Akani Sunny;Thaiföld;118500",
...
function ObjektumFeltolto(feltoltendoElem:string[]):snooker[]{
    ...
    for(let i:number=0; i<feltoltendoElem.length; i++){
        ...
        let objektum:snooker={
            pont:Number(daraboltSor[0]),
            nev:String(daraboltSor[1]),
...
function LegtobbNyeremeny(snookerAdatok:snooker[]):number{
    let legNagyobbNyeremeny:number=0;
    for(let i:number=0; snookerAdatok.length; i++){
          if(snookerAdatok[i].nyeremeny>legNagyobbNyeremeny){
            legNagyobbNyeremeny=snookerAdatok[i].nyeremeny;
          }

    }
    return legNagyobbNyeremeny;
}
//hát én  már nem értem miért nem működik :(